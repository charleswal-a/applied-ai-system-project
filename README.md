...
Building the original project showed me that a recommender is really just a math formula, a weighted sum of a few number categories and string comparisons, and that bias creeps in from decisions that seem small. Whoever picks the feature weights decides what "good taste" means to the system, so giving genre the highest weight meant genre mismatches were nearly impossible to overcome, and whoever builds the catalog decides whose taste gets served well, since a genre with one song can never produce a strong recommendation no matter what the user asks for. Adding the agentic layer on top didn't erase any of that, it just moved where the problem shows up. The guardrail now catches a contradiction or a missing genre before a result is shown instead of silently zeroing out a score, but it still can't decide what to do about it, and because it relies on an LLM to translate a request into those same rigid tags, it can introduce its own version of the same mismatch, like guessing "indie" instead of the catalog's actual "indie pop." Growing the catalog from 20 to 75 songs helped, but whoever curates the data is still deciding whose taste the system can actually serve well, no matter how much smarter the layer in front of it gets.